...
(function() {
	...
	var merrywrap = document.getElementById( 'merrywrap' ),
		...
		step = 1, stepTimes = [1000,1500,2300,6000];
	...
	function setGridDelays() {
		[].slice.call( merrywrap.querySelectorAll( '.row' ) ).forEach( function( row, i ) {
			...
			itemsrow.forEach( function( item, j ) {
				var delay = delays[ j ];
				item.style.webkitTransition = '-webkit-transform 1.4s ease-out ' + delay + 's, opacity 1.4s ' + delay + 's';
				item.style.transition = 'transform 1.4s ease-out ' + delay + 's, opacity 1.4s ' + delay + 's';
			} );
		} )
	}
